...
def calculate_heating_load(
    s_temperature: pd.Series, heating_on_temperature: float, annual_heat_kwh_consumption: float
) -> pd.Series:
...
def get_opex_estimate_ts_df(
    ...
    s_heating_load_kwh = calculate_heating_load(
        s_temperature, heating_on_temperature, annual_heat_kwh_consumption * n_households
    )
